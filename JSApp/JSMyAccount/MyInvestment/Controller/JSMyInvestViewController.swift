//
//  JSMyInvestViewController.swift
//  JSApp
//
...
class JSMyInvestViewController: BaseTableViewController,UITableViewDataSource,UITableViewDelegate {
    var investModel:MyInvestModel?      //我的投资数据 聚胜
    ...
    override func loadData() {
        ...
        MyInvestApi(Uid: UserModel.shareInstance.uid ?? 0, Status: 4, PageOn: pageIndex!).startWithCompletionBlock(success: { (request:YTKBaseRequest!) -> Void in
    ...
    //MARK: - 查看历史记录
    @IBAction func seeHistoryClick(_ sender: UIButton) {
        
        let historyRecordVC = JSHistoryInvestRecordViewController()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0
        {
            var cell = tableView.dequeueReusableCell(withIdentifier: "JSMyInvestFirstCell") as! JSMyInvestFirstCell!
            if cell == nil
            {
                cell = Bundle.main.loadNibNamed("JSMyInvestFirstCell", owner: self, options: nil)?.first as? JSMyInvestFirstCell
            }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return indexPath.row == 0 ? JSMyInvestFirstCell.cellHeight() : JSMyInvestListCell.cellHeight()
    }
    ...
    convenience  init() {
        let nibNameOrNil = String?("JSMyInvestViewController")
