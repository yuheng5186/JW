...
class BalancePaymentView: UIView {
    ...
    var controllerType:Int = 0         //用做友盟统计
    ...
    func setupContent()
    {
        ...
        contentView = UIView(frame: CGRect(x: 0, y: SCREEN_HEIGHT - contentViewH - TOP_HEIGHT, width: SCREEN_WIDTH, height: contentViewH))
        ...
        payButton.backgroundColor = DEFAULT_GREENCOLOR
    ...
    func dismissView() {
        ...
        if self.controllerType == 2 {
            MobClick.event("0400045")
        } else if self.controllerType == 3 {
            MobClick.event("0400064")
        }
