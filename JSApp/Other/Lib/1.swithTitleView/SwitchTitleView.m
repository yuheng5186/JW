...
@implementation SwitchTitleView
{
    ...
    NSLayoutConstraint *_titleBarConstraint;//_titleScroll的高度约束
...
- (void)creatScrollViews
{
    ...
    if (_iscreatSpecialBtn) {
        ...
        [_leftBtn setBackgroundImage:[UIImage imageNamed:@"united_states_flag"] forState:UIControlStateNormal];
        _leftBtn.translatesAutoresizingMaskIntoConstraints = NO;
        ...
        NSArray *leftBtnVConstrains =   [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-5-[leftBtn]-5-|" options:0 metrics:nil views:views];
        [_leftBGView addConstraints:leftBtnVConstrains];
        
        //1._titleScrollView水平上面的约束
        NSArray *HConstrains =  [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[leftBGView(60)]-0-[titleScrollView]-0-|" options:0 metrics:nil views:views];
...
- (void)createButtons
{
    ...
    if (a < titleViewWidth && (titleBtnMargin == 15)) {
       
        xOffset_reset = (titleViewWidth - a + 15.0f *(_viewArray.count + 1) ) /  (_viewArray.count + 1);//重新设置间隔
...
- (void)layoutSubviews
{
    if (_isInit) {
        ...
        //更新主视图的总宽度
        _mainScrollView.contentSize = CGSizeMake(CGRectGetWidth(self.frame) * _viewArray.count, 0);
        
        //更新_mainScrollView子视图的宽度
        for (int i = 0; i < [_viewArray count]; i++) {
            UIViewController *subVC = _viewArray[i];
            subVC.view.frame = CGRectMake(CGRectGetWidth(self.frame) * i, 0,CGRectGetWidth(self.frame), CGRectGetHeight(_mainScrollView.frame));
        }
        //滚动到选中的视图
