...
public enum tt_AlignType {
    ...
    fileprivate func layoutAttributes(_ isInner: Bool, isVertical: Bool) -> tt_LayoutAttributes {
        let attributes = tt_LayoutAttributes()
        ...
        case .centerCenter:     // 仅内部参照需要
            return tt_LayoutAttributes(horizontal: .centerX, referHorizontal: .centerX, vertical: .centerY, referVertical: .centerY)
        }
    }
}
...
extension UIView {
    ...
    ///  - parameter insets:    间距
    public func tt_Fill(_ referView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero) -> [NSLayoutConstraint] {
    ...
    ///
    ///  - returns: 约束数组
    public func tt_AlignInner(type: tt_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPoint.zero) -> [NSLayoutConstraint]  {
        
        return tt_AlignLayout(referView, attributes: type.layoutAttributes(true, isVertical: true), size: size, offset: offset)
    }
    ...
    ///
    ///  - returns: 约束数组
    public func tt_AlignVertical(type: tt_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPoint.zero) -> [NSLayoutConstraint] {
        
        return tt_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: true), size: size, offset: offset)
    }
    ...
    ///
    ///  - returns: 约束数组
    public func tt_AlignHorizontal(type: tt_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPoint.zero) -> [NSLayoutConstraint] {
        
        return tt_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: false), size: size, offset: offset)
    }
    ...
    ///
    ///  - returns: 约束数组
    public func tt_HorizontalTile(_ views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        firstView.tt_AlignInner(type: tt_AlignType.topLeft, referView: self, size: nil, offset: CGPoint(x: insets.left, y: insets.top))
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.tt_sizeConstraints(firstView)
            subView.tt_AlignHorizontal(type: tt_AlignType.topRight, referView: preView, size: nil, offset: CGPoint(x: insets.right, y: 0))
            preView = subView
        }
    ...
    ///
    ///  - returns: 约束数组
    public func tt_VerticalTile(_ views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.tt_sizeConstraints(firstView)
            subView.tt_AlignVertical(type: tt_AlignType.bottomLeft, referView: preView, size: nil, offset: CGPoint(x: 0, y: insets.bottom))
            preView = subView
        }
    ...
    public func tt_Constraint(_ constraintsList: [NSLayoutConstraint], attribute: NSLayoutAttribute) -> NSLayoutConstraint? {
    ...
    ///
    ///  - returns: 约束数组
    fileprivate func tt_AlignLayout(_ referView: UIView, attributes: tt_LayoutAttributes, size: CGSize?, offset: CGPoint) -> [NSLayoutConstraint] {
        
        translatesAutoresizingMaskIntoConstraints = false
        
        var cons = [NSLayoutConstraint]()
        
        cons += tt_positionConstraints(referView, attributes: attributes, offset: offset)
        
        if size != nil {
            cons += tt_sizeConstraints(size!)
        }
    ...
    ///
    ///  - returns: 约束数组
    fileprivate func tt_sizeConstraints(_ size: CGSize) -> [NSLayoutConstraint] {
    ...
    ///
    ///  - returns: 约束数组
    fileprivate func tt_sizeConstraints(_ referView: UIView) -> [NSLayoutConstraint] {
    ...
    ///
    ///  - returns: 约束数组
    fileprivate func tt_positionConstraints(_ referView: UIView, attributes: tt_LayoutAttributes, offset: CGPoint) -> [NSLayoutConstraint] {
...
///  布局属性
private final class tt_LayoutAttributes {
