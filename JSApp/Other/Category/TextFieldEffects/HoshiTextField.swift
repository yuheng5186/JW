...
@IBDesignable open class HoshiTextField: TextFieldEffects {
     ...
     */
    @IBInspectable dynamic open var placeholderColor: UIColor = DEFAULT_DARKGRAYCOLOR {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    override open func animateViewsForTextEntry() {
        if text!.isEmpty {
            UIView.animate(withDuration: 0.3, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 1.0, options: .beginFromCurrentState, animations: ({
                ...
                //lufeng 添加 更改placeholder文字
                self.placeholderLabel.text = (self.placeholder! as NSString).substring(from: 5)
